...
class Inventor_Notifications_Logic {
    ...
    public static function init() {
        add_action( 'init', array( __CLASS__, 'hook_post_statuses' ), 10 );
    ...
    public static function get_content_by_action( $action, $object ) {
        ...
        switch( $action ) {
            case INVENTOR_MAIL_ACTION_PACKAGE_WILL_EXPIRE:
                $args = array( 'package' => $object, 'threshold' => $threshold );
                $content = sprintf( __( 'Your package %s will expire in %d days.', 'inventor-notifications' ), $object, $threshold );
                break;
            case INVENTOR_MAIL_ACTION_PACKAGE_EXPIRED:
                $args = array( 'package' => $object );
            ...
            case INVENTOR_MAIL_ACTION_SUBMISSION_PENDING_ADMIN:
                $args = array( 'listing' => $object );
                $content = sprintf( __( 'New pending submission %s is waiting for admin review.' , 'inventor-notifications' ), $object );
                break;
            case INVENTOR_MAIL_ACTION_SUBMISSION_PENDING_USER:
                $args = array( 'listing' => $object );
                $content = sprintf( __( 'Your submission %s is waiting for admin review. Until then, it will be not published.' , 'inventor-notifications' ), $object );
                break;
            case INVENTOR_MAIL_ACTION_SUBMISSION_PUBLISHED_ADMIN:
                $args = array( 'listing' => $object );
    ...
    public static function create_package_expiration_notifications( $user_id ) {
        ...
        if ( class_exists( 'Inventor_Packages_Logic' ) ) {
            ...
            if ( $config['package_will_expire_enabled'] ) {
                ...
                self::create_notification( INVENTOR_MAIL_ACTION_PACKAGE_WILL_EXPIRE, $object, $user_id, $schedule_date );
            }
        }
    }
}
