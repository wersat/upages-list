...
class Inventor_Jobs_Logic {
    ...
    public static function init() {
        ...
        add_action( 'inventor_listing_detail', array( __CLASS__, 'render_total_job_applications' ), 1, 1 );
    ...
    public static function process_apply_form() {
        ...
        if ( $user_already_applied ) {
            $_SESSION['messages'][] = array( 'danger', __( 'You already applied for this job.', 'inventor-jobs' ) );
            return;
        }
        ...
        if ( ! empty( $status ) && 1 == $status ) {
            ...
            $_SESSION['messages'][] = array( 'success', __( 'Message has been successfully sent.', 'inventor-jobs' ) );
        } else {
            $_SESSION['messages'][] = array( 'danger', __( 'Unable to send a message.', 'inventor-jobs' ) );
        }
