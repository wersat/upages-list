...
class Inventor_Stripe_Logic {
    ...
    public static function process_payment() {
        ...
        try {
            ...
            echo wp_redirect( $redirect_url ); exit();

        } catch(\Stripe\Error\Card $e) {
            // The card has been declined
            $_SESSION['messages'][] = array( 'danger', __( 'The card has been declined.', 'inventor-stripe' ) );
    ...
    public static function process_result( $success, $settings, $payment_id, $token ) {
        ...
        if ( $success ) {
            if ( ! $is_valid ) {
                $_SESSION['messages'][] = array( 'danger', __( 'Payment is invalid.', 'inventor-stripe' ) );
            } else if ( ! in_array( $settings['payment_type'], apply_filters( 'inventor_payment_types', array() ) ) ) {
                $_SESSION['messages'][] = array( 'danger', __( 'Undefined payment type.', 'inventor-stripe' ) );
            } else {
                $_SESSION['messages'][] = array( 'success', __( 'Payment has been successful.', 'inventor-stripe' ) );
            }
        } else {
            $_SESSION['messages'][] = array( 'danger', __( 'Payment failed, canceled or is invalid.', 'inventor-stripe' ) );
        }
