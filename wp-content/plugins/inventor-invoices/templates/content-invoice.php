<div class="invoice">
  <?php
    ...
    $supplier_registration_number = get_post_meta($invoice_id, INVENTOR_INVOICE_PREFIX . 'supplier_registration_number',
      true);
    ...
    $customer_registration_number = get_post_meta($invoice_id, INVENTOR_INVOICE_PREFIX . 'customer_registration_number',
      true);
  ...
  <section id="general">
    <dl>
      <dt><?php echo __('Issue date', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo $issue_date; ?></dd>
      <dt><?php echo __('Payment term', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo _n(sprintf('%d day', $payment_term), sprintf('%d days', $payment_term), $payment_term,
          'inventor-invoices'); ?></dd>
      <dt><?php echo __('Reference', 'inventor-invoices') . ':' ?></dt>
  ...
  <?php if ( ! empty($details)): ?>
    <section id="details">
      <dl>
        <dt><?php echo __('Details / Instructions', 'inventor-invoices') . ':' ?></dt>
        <dd><?php echo nl2br(apply_filters('inventor_invoices_payment_details', $details)); ?></dd>
      </dl>
    </section>
  <?php endif; ?>
  <section id="supplier">
    ...
    <dl>
      <dt><?php echo __('Name', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo $supplier_name; ?></dd>
      <dt><?php echo __('Reg. No.', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo $supplier_registration_number; ?></dd>
      <dt><?php echo __('VAT No.', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo $supplier_vat_number; ?></dd>
      <dt><?php echo __('Details', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo nl2br(apply_filters('inventor_invoices_supplier_details', $supplier_details)); ?></dd>
    </dl>
  </section>
  <section id="customer">
    ...
    <dl>
      ...
      <dt><?php echo __('Reg. No.', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo $customer_registration_number; ?></dd>
      <dt><?php echo __('VAT No.', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo $customer_vat_number; ?></dd>
      <dt><?php echo __('Details', 'inventor-invoices') . ':' ?></dt>
      <dd><?php echo nl2br(apply_filters('inventor_invoices_customer_details', $customer_details)); ?></dd>
    </dl>
  </section>
  <section id="items">
    <?php if (count($items) > 0) : ?>
      <table class="invoice-items-table">
        <thead>
          <th><?php echo __('Item', 'inventor-invoice'); ?></th>
          <th><?php echo __('Quantity', 'inventor-invoice'); ?></th>
          <th><?php echo __('Unit price', 'inventor-invoice'); ?></th>
          <th><?php echo __('Tax rate', 'inventor-invoice'); ?></th>
          <th><?php echo __('Subtotal', 'inventor-invoice'); ?></th>
          <th><?php echo __('Currency', 'inventor-invoice'); ?></th>
        </thead>
        <tbody>
  ...
  <section id="total">
    <label><?php echo __('Total', 'inventor-invoices') . ':' ?></label> <span><?php printf('%s %s', $total,
        $currency_code); ?>
      <span>
  </section>
</div>
