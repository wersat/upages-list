<?php
    ...
    class Inventor_PayPal_Logic
    {
        ...
        public static function init()
        {
            ...
            add_action('init', [__CLASS__, 'process_result'], 9999);
            ...
            if (self::is_active()) {
                if (get_theme_mod('inventor_paypal_credit_card', false) == '1') {
                    $gateways[] = [
                        ...
                        'proceed' => true,
                        'content' => Inventor_Template_Loader::load('paypal/credit-card-form', [], INVENTOR_PAYPAL_DIR),
                    ];
                }
                $gateways[] = [
                    'id'      => 'paypal-account',
            ...
            switch ($gateway) {
                case 'paypal-credit-card':
                    if (empty($_POST['first_name'])) {
                        $_SESSION['messages'][] = ['danger', __('First name is required.', 'inventor-paypal')];
                        break;
                    }
                    if (empty($_POST['last_name'])) {
                        $_SESSION['messages'][] = ['danger', __('Last name is required.', 'inventor-paypal')];
                        break;
                    }
                    if (empty($_POST['card_number'])) {
                        $_SESSION['messages'][] = ['danger', __('Card number is required.', 'inventor-paypal')];
                        break;
                    }
                    if (empty($_POST['cvv'])) {
                        $_SESSION['messages'][] = ['danger', __('CVV is required.', 'inventor-paypal')];
                        break;
                    }
                    if (empty($_POST['expires_month'])) {
                        $_SESSION['messages'][] = ['danger', __('Expires month is required.', 'inventor-paypal')];
                        break;
                    }
                    if (empty($_POST['expires_year'])) {
                        $_SESSION['messages'][] = ['danger', __('Expires year is required.', 'inventor-paypal')];
                        break;
                    }
                    if ( ! self::validate_card_number($_POST['card_number'])) {
                        $_SESSION['messages'][] = ['danger', __('Credit card number is not valid.', 'inventor-paypal')];
                        break;
                    }
                    if ( ! self::validate_cvv($_POST['cvv'])) {
                        $_SESSION['messages'][] = ['danger', __('CVV number is not valid.', 'inventor-paypal')];
                        break;
                    }
                    ...
                    if ( ! empty($payment->state)) {
                        ...
                        if (in_array($payment->state, ['approved', 'completed'])) {
                            $url = self::get_paypal_url(true, $gateway, get_current_user_id(),
                                $settings['payment_type'], $settings['object_id'], $settings['billing_details'],
                                $payment->id);
                            wp_redirect($url);
                            exit();
                        }
                        if (in_array($payment->state, ['failed', 'canceled', 'expired'])) {
                            $url = self::get_paypal_url(false, $gateway, get_current_user_id(),
                                $settings['payment_type'], $settings['object_id'], $settings['billing_details'],
                                $payment->id);
                            wp_redirect($url);
                            exit();
                        ...
                        $url = self::get_paypal_url(false, $gateway, get_current_user_id(), $settings['payment_type'],
                            $settings['object_id'], $settings['billing_details'], $payment->id);
                        wp_redirect($url);
                        exit();
                    }
                    break;
        ...
        public static function process_result()
        {
            ...
            if ($success) {
                ...
                $paypal_params = $_GET['gateway'] == 'paypal-account' ? ['paymentId', 'token', 'PayerID']
                    : ['paymentId'];
                ...
                if ($is_valid) {
                    if ($_GET['gateway'] == 'paypal-account') {
                        $is_valid = self::is_paypal_payment_valid($_GET['gateway'], $_GET['paymentId'], $_GET['token'],
                            $_GET['PayerID']);
                    } else {
            ...
            $params = $_GET;
            foreach (['success', 'gateway', 'payment_type', 'object_id', 'user_id'] as $unset_key) {
                unset($params[$unset_key]);
            }
            // create transaction
            Inventor_Post_Type_Transaction::create_transaction($_GET['gateway'], $success, $_GET['user_id'],
                $_GET['payment_type'], $_GET['paymentId'], $_GET['object_id'], $_GET['price'], $_GET['currency_code'],
                $params);
            // billing_details
            $billing_details = Inventor_Billing::get_billing_details_from_context($_GET);
            // hook inventor action
            do_action('inventor_payment_processed', $success, $_GET['gateway'], $_GET['payment_type'],
                $_GET['paymentId'], $_GET['object_id'], $_GET['price'], $_GET['currency_code'], $_GET['user_id'],
                $billing_details);
            // handle payment
            if ($success) {
                if ( ! $is_valid) {
                ...
                $_SESSION['messages'][] = ['danger', __('Payment failed, canceled or is invalid.', 'inventor-paypal')];
            }
            // after payment page
        ...
        public static function process_credit_card(array $settings)
        {
            ...
            try {
                ...
                $_SESSION['messages'][] = ['danger', __('There was an error processing payment.', 'inventor-paypal')];
        ...
        public static function get_account_approval_url(array $settings)
        {
            ...
            $return_url   = self::get_paypal_url(true, 'paypal-account', get_current_user_id(),
                $settings['payment_type'], $settings['object_id'], $settings['billing_details']);
            $cancel_url   = self::get_paypal_url(false, 'paypal-account', get_current_user_id(),
                $settings['payment_type'], $settings['object_id'], $settings['billing_details']);
