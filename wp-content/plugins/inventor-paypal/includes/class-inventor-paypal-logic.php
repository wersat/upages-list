...
class Inventor_PayPal_Logic {
        ...
        switch ( $gateway ) {
            case 'paypal-credit-card':
                if ( empty( $_POST['first_name']) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'First name is required.', 'inventor-paypal' ) );
                    break;
                }

                if ( empty( $_POST['last_name']) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'Last name is required.', 'inventor-paypal' ) );
                    break;
                }

                if ( empty( $_POST['card_number']) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'Card number is required.', 'inventor-paypal' ) );
                    break;
                }

                if ( empty( $_POST['cvv']) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'CVV is required.', 'inventor-paypal' ) );
                    break;
                }

                if ( empty( $_POST['expires_month']) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'Expires month is required.', 'inventor-paypal' ) );
                    break;
                }

                if ( empty( $_POST['expires_year']) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'Expires year is required.', 'inventor-paypal' ) );
                    break;
                }

                if ( ! self::validate_card_number( $_POST['card_number'] ) ){
                    $_SESSION['messages'][] = array( 'danger', __( 'Credit card number is not valid.', 'inventor-paypal' ) );
                    break;
                }

                if ( ! self::validate_cvv( $_POST['cvv'] ) ){
                    $_SESSION['messages'][] = array( 'danger', __( 'CVV number is not valid.', 'inventor-paypal' ) );
                    break;
                }

                if ( $terms && empty( $_POST['agree_terms'] ) ) {
                    $_SESSION['messages'][] = array( 'danger', __( 'You must agree terms &amp; conditions.', 'inventor-paypal' ) );
                    break;
                }
    ...
    public static function process_result() {
        ...
        if ( $success ) {
            if( ! $is_valid ) {
                $_SESSION['messages'][] = array( 'danger', __( 'Payment is invalid.', 'inventor-paypal' ) );
            } else if ( ! in_array( $_GET['payment_type'], apply_filters( 'inventor_payment_types', array() ) ) ) {
                $_SESSION['messages'][] = array( 'danger', __( 'Undefined payment type.', 'inventor-paypal' ) );
            } else {
                $_SESSION['messages'][] = array( 'success', __( 'Payment has been successful.', 'inventor-paypal' ) );
            }
        } else {
            $_SESSION['messages'][] = array( 'danger', __( 'Payment failed, canceled or is invalid.', 'inventor-paypal' ) );
        }
    ...
    public static function process_credit_card( array $settings ) {
        ...
        try {
            ...
            $_SESSION['messages'][] = array( 'danger', __( 'There was an error processing payment.', 'inventor-paypal' ) );
            return $ex;
        }
    }
