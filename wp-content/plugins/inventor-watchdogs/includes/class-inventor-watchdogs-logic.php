...
class Inventor_Watchdogs_Logic {
    ...
    public static function get_lookup_name_by_key( $key ) {
        $key = str_replace( array( 'filter-' ), array( '' ), $key);
    ...
    public static function add_watchdog() {
        ...
        $_SESSION['messages'][] = array( $message_level, $result['message'] );
    ...
    public static function remove_watchdog() {
        ...
        if ( isset( $_GET[ 'id' ] ) ) {
            ...
            if ( empty( $watchdog ) ) {
                $_SESSION['messages'][] = array( 'danger', __( 'Watchdog does not exist!', 'inventor-watchdogs' ) );
            } else if( $watchdog->post_author != get_current_user_id() ) {
                $_SESSION['messages'][] = array( 'danger', __( 'It is not your watchdog!', 'inventor-watchdogs' ) );
            } else {
                wp_delete_post( $_GET[ 'id' ] );
                $_SESSION['messages'][] = array( 'success', __( "Watchdog deleted.", 'inventor-watchdogs' ));
            }
        } else {
