		...
		foreach ( $property_groups as $group ) {
			...
			if ( have_posts() ) {
				...
				if ( count( $group ) > 1 ) {
					...
					// Marker
					ob_start();
					$template = Inventor_Template_Loader::locate( 'google-map/infowindow-group', INVENTOR_GOOGLE_MAP_DIR );
					include( $template );
					...
					// Infowindow
					ob_start();
					$template = Inventor_Template_Loader::locate( 'google-map/marker-group', INVENTOR_GOOGLE_MAP_DIR );
					include( $template );
					$output = ob_get_contents();
					ob_end_clean();
