...
class CMB2_Field_Taxonomy_Multicheck_Hierarchy
{
    ...
    {
        add_filter('cmb2_render_taxonomy_multicheck_hierarchy', [$this, 'render'], 10, 5);
        add_filter('cmb2_sanitize_taxonomy_multicheck_hierarchy', [$this, 'sanitize'], 10, 5);
    }
    ...
    public function render($field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object)
    {
        ...
        $terms       = get_terms(
            $field_type_object->field->args('taxonomy'), [
            'hide_empty' => false,
            'parent'     => 0,
            ]
        );
        ...
        $classes = false === $field_type_object->field->args('select_all_button')
            ? 'cmb2-checkbox-list no-select-all cmb2-list' : 'cmb2-checkbox-list cmb2-list';
