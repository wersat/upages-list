...
if (! class_exists('Taxonomy_MetaData')) :
    ...
    class Taxonomy_MetaData
    {
       ...
       * @var   array
       */
        protected static $taxonomy_objects = [];
       ...
       * @since 0.1.0
       */
        public function __construct($taxonomy, $fields, $title = '', $option_callbacks = [])
        {
       ...
       */
        public function hooks()
        {
            ...
            add_action("{$this->taxonomy}_edit_form", [$this, 'metabox_edit'], 8, 2);
            ...
            if (apply_filters("taxonomy_metadata_display_on_{$this->taxonomy}_add_form", true)) {
                add_action("{$this->taxonomy}_add_form_fields", [$this, 'metabox_edit'], 8, 2);
            }
            // Save our form data
            add_action("created_{$this->taxonomy}", [$this, 'save_data']);
            add_action("edited_{$this->taxonomy}", [$this, 'save_data']);
            // Delete it if necessary
            add_action("delete_{$this->taxonomy}", [$this, 'delete_data']);
        }
       ...
       */
        public function display_form($term_id)
        {
            ...
            if ($this->section_title) : ?>
                <h3 class="cmb-metabox-title"><?php echo $this->section_title;
                ?></h3>
            <?php endif;
            ?>
          ...
          <table class="form-table term-meta-box">
            ...
            $this->loop_fields($data, [$this, 'render_field_view']);
            ?>
            </table>
            <?php
       ...
       */
        public function text_input_view($field, $value)
        {
            ...
            printf(
                '<input %s type="text" class="regular-text" name="%s" id="%s" value="%s" />%s', $placeholder,
                esc_attr($field->id), esc_attr($field->id), esc_attr($value), $description
            );
        }
       ...
       */
        public function save_data($term_id)
        {
            ...
            $this->id = $this->id_base . '_' . $term_id : $this->id($term_id);
       ...
       */
        ...
        {
            if (! isset($_POST['term_opt_name'], $_POST['term_meta_box_nonce'], $_POST['action'])
            ...
            $this->loop_fields($_POST, [$this, 'sanitize_field']);
            // Save the field data
       ...
       */
        ...
        {
            $cb                          = isset($field->sanitize_cb) && is_callable($field->sanitize_cb)
            ? $field->sanitize_cb
            : 'sanitize_text_field';
