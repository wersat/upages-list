...
<div class="listing-row <?php if ( ! empty( $featured ) && 'on' == $featured ) : ?>listing-featured<?php endif; ?> <?php if ( ! empty( $reduced ) && 'on' == $reduced ) : ?>listing-reduced<?php endif; ?>">
    <div class="listing-row-image">
        <?php if ( has_post_thumbnail() ) : ?>
            ...
            <a href="<?php the_permalink(); ?>"  style="background-image: url('<?php echo esc_attr( $image[0] ); ?>');"></a><!-- /.listing-row-image -->
        <?php else : ?>
            <a href="<?php the_permalink(); ?>" class="listing-row-image" style="background-image: url('<?php echo plugins_url( 'inventor' ); ?>/assets/img/default-item.png');"></a><!-- /.listing-row-image -->
        <?php endif; ?>
