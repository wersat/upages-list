  ...
  <?php if (!empty($instance['result_numbers_enabled'])) : ?>
    ...
    <?php if (Inventor_Filter::has_filter(true) && $total != 0 && $matches != 0 and $total != $matches) : ?>
      <h3 class="filter-result-numbers">
        ...
        <?php else: ?>
          <?php printf(_n('<span class="match">%d</span> listing matches your search criteria',
            '<span class="match">%d</span> listings match your search criteria', $matches, 'inventor'), $matches); ?>
        <?php endif; ?>
      </h3>
