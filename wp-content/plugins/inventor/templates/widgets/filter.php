...
<div class="widget-inner
    ...
    <?php require Inventor_Template_Loader::locate('widgets/filter-form'); ?>
    ...
    <?php if (Inventor_Filter::has_filter(true) && $total !== 0 && $matches !== 0 and $total !== $matches) : ?>
      <h3 class="filter-result-numbers">
        <?php if (! empty($instance['result_numbers_total'])) : ?>
            <?php printf(
                _n(
                    '<span class="match">%d</span> listing from <span class="total">%d</span> matches your search criteria',
        ...
        <?php else: ?>
            <?php printf(
                _n(
                    '<span class="match">%d</span> listing matches your search criteria',
                    '<span class="match">%d</span> listings match your search criteria', $matches, 'inventor'
                ), $matches
            ); ?>
        <?php endif; ?>
      </h3>
