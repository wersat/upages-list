<?php
    ...
    class Inventor_Submission
    {
        ...
        {
            add_action('init', [__CLASS__, 'process_remove_form'], 9999);
            add_action('cmb2_init', [__CLASS__, 'process_submission'], 9999);
            add_action('transition_post_status', [__CLASS__, 'update_general_metabox_fields'], 10, 3);
        }
        ...
        {
            $valid_type = !empty($_GET['type'])
                          && in_array($_GET['type'], Inventor_Post_Types::get_listing_post_types());
            if ($valid_type) {
        ...
        public static function process_submission_save($post_type)
        {
            ...
            $description = self::get_submission_field_value(INVENTOR_LISTING_PREFIX.$post_type.'_general',
                INVENTOR_LISTING_PREFIX.'description');
            $data = [
                'post_title' => sanitize_text_field(self::get_submission_field_value(INVENTOR_LISTING_PREFIX.$post_type.'_general',
            ...
            if (!empty($post_id)) {
                ...
                if (!empty($_SESSION['submission'])) {
                    ...
                    $featured_image_id
                        = self::get_submission_field_value(INVENTOR_LISTING_PREFIX.$post_type.'_general',
                        INVENTOR_LISTING_PREFIX.'featured_image_id');
                ...
                if ($new_post) {
                    ...
                    $_SESSION['messages'][] = [
                        'success',
                        __('New submission has been successfully created.', 'inventor'),
                    ];
                } else {
                    // action
                    do_action('inventor_submission_listing_updated', $post_id, $post_type);
                    // message
                    $_SESSION['messages'][] = ['success', __('Submission has been successfully updated.', 'inventor')];
                }
            }
        ...
        public static function update_general_metabox_fields($new_status, $old_status, $post)
        {
            ...
            update_post_meta($post->ID, INVENTOR_LISTING_PREFIX.'featured_image',
        ...
        public static function process_remove_form()
        {
            ...
            if (wp_delete_post($_POST['listing_id'])) {
                $_SESSION['messages'][] = ['success', __('Item has been successfully removed.', 'inventor')];
            } else {
                $_SESSION['messages'][] = ['danger', __('An error occurred when removing an item.', 'inventor')];
            }
        }
    }
