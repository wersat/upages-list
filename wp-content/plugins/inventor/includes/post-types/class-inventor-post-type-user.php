...
class Inventor_Post_Type_User {
	...
	public static function process_profile_form() {
		if ( ! empty( $_POST['submit-profile'] ) ) {
			...
			$_SESSION['messages'][] = array( 'success', __( 'Profile has been successfully updated.', 'inventor' ) );
	...
	public static function process_change_password_form() {
		...
		if ( empty( $old_password ) || empty( $new_password ) || empty( $retype_password ) ) {
			$_SESSION['messages'][] = array( 'warning', __( 'All fields are required.', 'inventor' ) );
			return;
		}

		if ( $new_password != $retype_password ) {
			$_SESSION['messages'][] = array( 'warning', __( 'New and retyped password are not same.', 'inventor' ) );
		}

		$user = wp_get_current_user();

		if ( ! wp_check_password( $old_password, $user->data->user_pass, $user->ID ) ) {
			$_SESSION['messages'][] = array( 'warning', __( 'Your old password is not correct.', 'inventor' ) );
			return;
		}

		wp_set_password( $new_password, $user->ID );
		$_SESSION['messages'][] = array( 'success', __( 'Your password has been successfully changed.', 'inventor' ) );
	}
	...
	public static function process_login_form() {
		...
		if ( empty( $_POST['login'] ) || empty( $_POST['password'] ) ) {
			$_SESSION['messages'][] = array( 'warning', __( 'Login and password are required.', 'inventor' ) );
		...
		$_SESSION['messages'][] = array( 'success', __( 'You have been successfully logged in.', 'inventor' ) );
	...
	public static function process_reset_password_form() {
		...
		if ( is_wp_error( $result ) ) {
			$_SESSION['messages'][] = array( 'danger', __( 'Unable to send an e-mail.', 'inventor' ) );
		} else {
			$_SESSION['messages'][] = array( 'success', __( 'Please check inbox for more information.', 'inventor' ) );
		}
	...
	public static function process_register_form() {
		...
		if ( empty( $_POST['name'] ) || empty( $_POST['email'] ) ) {
			$_SESSION['messages'][] = array( 'danger', __( 'Username and e-mail are required.', 'inventor' ) );
			wp_redirect( $_SERVER['HTTP_REFERER'] );
			exit();
		}
		...
		if ( ! empty( $user_id ) ) {
			$_SESSION['messages'][] = array( 'danger', __( 'Username already exists.', 'inventor' ) );
			wp_redirect( $_SERVER['HTTP_REFERER'] );
			exit();
		}
		...
		if ( ! empty( $user_id ) ) {
			$_SESSION['messages'][] = array( 'danger', __( 'Email already exists.', 'inventor' ) );
			wp_redirect( $_SERVER['HTTP_REFERER'] );
			exit();
		}

		if ( $_POST['password'] != $_POST['password_retype'] ) {
			$_SESSION['messages'][] = array( 'danger', __( 'Passwords must be same.', 'inventor' ) );
			wp_redirect( $_SERVER['HTTP_REFERER'] );
			exit();
		}
		...
		if ( $terms_id && empty( $_POST['agree_terms'] ) ) {
			$_SESSION['messages'][] = array( 'danger', __( 'You must agree terms &amp; conditions.', 'inventor' ) );
			wp_redirect( $_SERVER['HTTP_REFERER'] );
			exit();
		}
