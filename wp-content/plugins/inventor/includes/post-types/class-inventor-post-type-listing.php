...
class Inventor_Post_Type_Listing {
	...
	public static function process_inquire_form() {
		...
		do_action( 'inventor_inquire_message_sent', $success,
			$_POST['post_id'], $_POST['name'], $_POST['email'], $_POST['subject'], $_POST['date'], $_POST['message'],
			! empty( $_POST['receive_author'] ), ! empty( $_POST['receive_admin'] ), ! empty( $_POST['receive_listing_email'] )
		);

		if ( $success ) {
			$_SESSION['messages'][] = array( 'success', __( 'Message has been successfully sent.', 'inventor' ) );
		} else {
			$_SESSION['messages'][] = array( 'danger', __( 'Unable to send a message.', 'inventor' ) );
		}
