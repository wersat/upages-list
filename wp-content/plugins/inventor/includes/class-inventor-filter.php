    ...
    public static function filter_query($query = null, $params = null)
    {
        ...
        if (! empty($params['filter-title'])) {
            $title_ids
                 = $wpdb->get_col(
                     $wpdb->prepare(
                         "SELECT DISTINCT ID FROM {$wpdb->posts} WHERE post_status = \"publish\" AND post_title LIKE '%s'",
                         '%' . $params['filter-title'] . '%'
                     )
                 );
        ...
        if (! empty($params['filter-description'])) {
            $description_ids
                 = $wpdb->get_col(
                     $wpdb->prepare(
                         "SELECT DISTINCT ID FROM {$wpdb->posts} WHERE post_status = \"publish\" AND post_content LIKE '%s'",
        ...
        if (! empty($params['filter-keyword'])) {
            $keyword_ids
                 = $wpdb->get_col(
                     $wpdb->prepare(
                         "SELECT DISTINCT ID FROM {$wpdb->posts} WHERE post_status = \"publish\" AND post_content LIKE '%s' OR post_title LIKE '%s'",
        ...
        if (! empty($params['filter-distance-latitude']) && ! empty($params['filter-distance-longitude']) && ! empty($params['filter-distance'])) {
            $distance_ids = [];
            $rows         = self::filter_by_distance(
                $params['filter-distance-latitude'],
