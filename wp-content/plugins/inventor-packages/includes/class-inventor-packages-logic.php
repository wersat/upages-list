...
class Inventor_Packages_Logic {
    ...
    public static function init() {
        ...
        add_filter( 'inventor_submission_listing_metabox_allowed', array( __CLASS__, 'submission_listing_metabox_allowed' ), 10, 3 );
        add_filter( 'inventor_pricing_table_items', array( __CLASS__, 'pricing_table_items' ), 10, 2 );
    ...
    public static function process_free_package_selection() {
        ...
        if ( ! self::is_package_free( $object_id ) ) {
            $_SESSION['messages'][] = array( 'danger', __( 'Package is not free.', 'inventor-packages' ) );
            wp_redirect( site_url() );
            exit();
        }

        if ( self::set_package_for_user( $user_id, $object_id ) ) {
            $_SESSION['messages'][] = array( 'success', __( 'Package has been set.', 'inventor-packages' ) );
        } else {
            $_SESSION['messages'][] = array( 'danger', __( 'Could not set package for user.', 'inventor-packages' ) );
        }
    ...
    public static function catch_payment( $success, $gateway, $payment_type, $payment_id, $object_id, $price, $currency_code, $user_id, $billing_details ) {
        if ( $success && $payment_type == 'package' && $gateway != 'wire-transfer' ) {
            ...
            $_SESSION['messages'][] = array( 'success', __( 'Package has been upgraded.', 'inventor-packages' ) );
        }
    }
