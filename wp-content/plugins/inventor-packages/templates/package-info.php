...
<div class="package-info-wrapper">
  ...
  <?php if (empty($current_package)) : ?>
    <?php if (get_theme_mod('inventor_submission_type', 'free') == 'packages') : ?>
      <p class="package-info">
        <?php echo __('Site is using packages. Before you can post listing, please upgrade your package.',
          'inventor-packages'); ?>
      </p>
    <?php else: ?>
      <p class="package-info">
  ...
  <?php else : ?>
    <div class="package-info">
      <p><?php echo sprintf(__('You are using <strong>%s</strong> package.', 'inventor-packages'),
          esc_attr($current_package->post_title)); ?></p>
      ...
      <?php if (Inventor_Packages_Logic::is_package_valid_for_user(get_current_user_id())) : ?>
        <p>
          <?php if ($date): ?>
            <?php echo sprintf(__('Your membership is valid until <strong>%s</strong>.', 'inventor-packages'),
              esc_attr($date)); ?>
          <?php endif; ?>
          <?php if (get_theme_mod('inventor_submission_type', 'free') == 'packages') : ?>
            ...
            <?php elseif ((int)$remaining < 0) : ?>
              <?php echo sprintf(__('You can not add new listings because you spent all of your available listings. Change your package. First <strong>%s</strong> items has been disabled from listing.',
                'inventor-packages'), esc_attr(abs($remaining))); ?>
            <?php else : ?>
              <?php echo sprintf(__('You can add <strong>%s</strong> items.', 'inventor-packages'),
                esc_attr($remaining)); ?>
            <?php endif; ?>
          <?php endif; ?>
        </p>
      <?php else : ?>
        <p>
          <?php echo sprintf(__('Your membership already expired at <strong>%s</strong>.', 'inventor-packages'),
            $date); ?>
          <?php if (get_theme_mod('inventor_submission_type', 'free') == 'packages') : ?>
            <?php echo sprintf(__('All your items has been deactivated until you pay for new membership.',
              'inventor-packages'), $date); ?>
          <?php endif; ?>
        </p>
      <?php endif; ?>
    </div>
  ...
  <?php $package_payment_id = get_theme_mod('inventor_general_payment_page', false); ?>

  <?php if ( ! $package_payment_id) : ?>
    <p><?php echo __('Payment page has been not set.', 'inventor-packages'); ?></p>
  <?php endif; ?>
  ...
  <?php if ( ! empty($packages) && ! empty($package_payment_id)) : ?>
    <form method="post" action="<?php echo esc_attr(get_permalink($package_payment_id)); ?>">
      ...
      <button type="submit" class="btn btn-primary btn-block" name="change-package"><?php echo __('Change',
          'inventor-packages'); ?></button>
    </form>
  <?php endif; ?>
</div>
