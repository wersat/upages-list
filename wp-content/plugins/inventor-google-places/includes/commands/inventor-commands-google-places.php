<?php
    ...
    /**
     * Class Inventor_Google_Places_Comannds_Google_Places.
     */
    class Inventor_Google_Places_Commands_Google_Places extends WP_CLI_Command
    {
        ...
        public $status_codes
            = [
                ...
                'OVER_QUERY_LIMIT' => 'You are over quota.',
                'inventor-google-places',
         ...
         * : Google Maps API key.
         * <output>
         * : Filename containg the results. The directory is wp-contnet/inventor-google-places
         * ## EXAMPLES
         *     wp inventor-google-places radasearch --location=40.738095,-73.991360 --radius=500 --api-key=YourApiKey --output=results.csv
         ...
         * @param int         $sleep
         */
        private function parse($args, $assoc_args, $index = 1, $content = [], $next_page = null, $sleep = 3)
        {
            ...
            foreach ($json->results as $result) {
                $detail
                        = wp_remote_get('https://maps.googleapis.com/maps/api/place/details/json?reference=' . $result->reference . '&key=' . $assoc_args['api-key']);
                $detail = json_decode($detail['body']);
                $content[] = [
                    'name'                       => ! empty($detail->result->name) ? $detail->result->name : null,
                    'latitude'                   => ! empty($detail->result->geometry)
                        ? $detail->result->geometry->location->lat
                        : null,
                    'longitude'                  => ! empty($detail->result->geometry)
                        ? $detail->result->geometry->location->lng
                        : null,
                    'rating'                     => ! empty($detail->result->rating) ? $detail->result->rating : null,
                    'price_level'                => ! empty($detail->result->price_level) ? $detail->result->price_level
                        : null,
                    'icon'                       => ! empty($detail->result->icon) ? $detail->result->icon : null,
                    'types'                      => ! empty($detail->result->types) ? implode($detail->result->types,
                        ',')
                        : null,
                    'formatted_address'          => ! empty($detail->result->formatted_address)
                        ? $detail->result->formatted_address : null,
                    'vicinity'                   => ! empty($detail->result->vicinity) ? $detail->result->vicinity
                        : null,
                    'url'                        => ! empty($detail->result->url) ? $detail->result->url : null,
                    'website'                    => ! empty($detail->result->website) ? $detail->result->website : null,
                    'formatted_phone_number'     => ! empty($detail->result->formatted_phone_number)
                        ? $detail->result->formatted_phone_number : null,
                    'international_phone_number' => ! empty($detail->result->international_phone_number)
                        ? $detail->result->international_phone_number : null,
                    'id'                         => $detail->result->id,
                    'attribution'                => ! empty($detail->result->html_attributions) ? implode(',',
                        $detail->result->html_attributions) : null,
                ];
            }
