...
<div class="detail-banner"
  ...
  <?php if ('banner_image' == $listing_banner) : ?>
    data-background-image="<?php echo esc_attr(get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'banner_image',
      true)); ?>"
  <?php endif; ?>
>
  <?php // Video. ?>
  <?php if ('banner_video' == $listing_banner) : ?>
    ...
    <video autoplay muted
      ...
      <source src="<?php echo esc_attr(get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'banner_video',
        true)); ?>" type="video/mp4">
      <div class="alert alert-danger"><?php esc_attr__('Your browser does not support the video html tag.',
          'superlist'); ?></div>
    </video>
  <?php endif; ?>
  ...
  <?php if ('banner_street_view' == $listing_banner && ! empty($street_view)) : ?>
    <?php $banner_latitude = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'street_view_location_latitude',
      true); ?>
    <?php $banner_longitude = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'street_view_location_longitude',
      true); ?>
    <?php $banner_zoom = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'street_view_location_zoom', true); ?>
    <?php $banner_heading = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'street_view_location_heading',
      true); ?>
  ...
  <?php if ('banner_inside_view' == $listing_banner && ! empty($inside_view)) : ?>
    <?php $banner_latitude = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'inside_view_location_latitude',
      true); ?>
    <?php $banner_longitude = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'inside_view_location_longitude',
      true); ?>
    <?php $banner_zoom = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'inside_view_location_zoom', true); ?>
    <?php $banner_heading = get_post_meta(get_the_ID(), INVENTOR_LISTING_PREFIX . 'inside_view_location_heading',
      true); ?>
    ...
    <div class="detail-banner-right">
      ...
      <?php if ( ! empty($price)) : ?>
        <div class="detail-banner-price <?php echo ( ! empty($reduced)) ? 'reduced-price' : ''; ?>">
          <span class="detail-banner-price-label"><?php echo esc_attr(! empty($reduced)) ? esc_attr__('Reduced Price',
              'superlist') : esc_attr__('Price', 'superlist'); ?>:</span>
