...
<div class="listing-row <?php if ( ! empty( $featured ) ) : ?>featured<?php endif; ?>">
    <div  class="listing-row-image"
        ...
        <?php if ( ! empty( $listing_category ) ) : ?>
            <div class="listing-row-label-top"><?php echo wp_kses( $listing_category, wp_kses_allowed_html( 'post' ) ); ?></div><!-- /.listing-row-label-top -->
        <?php endif; ?>

        <?php $price = Inventor_Price::get_price(); ?>
        <?php if ( ! empty( $price ) ) : ?>
            <div class="listing-row-label-bottom"><?php echo wp_kses( $price, wp_kses_allowed_html( 'post' ) ); ?></div><!-- /.listing-row-label-bottom -->
        <?php endif; ?>
    </div><!-- /.listing-row-image -->


    <div class="listing-row-body">
        <h2 class="listing-row-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
        <div class="listing-row-content">
            <?php the_excerpt(); ?>

            <p>
                <a class="read-more-link" href="<?php echo esc_attr( get_permalink( get_the_ID() ) ); ?>"><?php echo esc_attr__( 'Read More', 'superlist' ); ?><i class="fa fa-chevron-right"></i></a>
            </p>
