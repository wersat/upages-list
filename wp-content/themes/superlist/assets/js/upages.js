jQuery(document).ready(function($) {
  ...
  if (listingDetailMenu.length) {
     ...
     * Listing menu scroll
     */
    listingDetailMenu.find('a').click(function(e) {
      e.preventDefault();
     ...
     * Listing menu affix
     */
    if (listingDetailMenu.length !== 0) {
      var height = listingDetailMenu.offset().top;
     ...
     */
    $('body').scrollspy({
      target: '.listing-detail-menu', offset: 160
    });
  }
  ...
  if (listingDetailPrice.length) {
    var height = listingDetailPrice.offset().top;
  ...
  if (simple_map.length) {
    ...
    simple_map.google_map({
      center: {
        latitude: simple_map.data('latitude'), longitude: simple_map.data('longitude')
      },
  ...
  if (banner_map.length) {
    ...
    if (banner_map.data('marker')) {
      ...
      markers = [
        {
          latitude: banner_map.data('latitude'), longitude: banner_map.data('longitude'), marker_content: markerContent
        }
      ]
    }
    ...
    banner_map.google_map({
      center: {
        latitude: banner_map.data('latitude'), longitude: banner_map.data('longitude')
      },
  ...
  function streetViewInit(banner_street_view_id) {
    ...
    if (banner_street_view.length) {
      new google.maps.StreetViewPanorama(document.getElementById(banner_street_view_id), {
        position: {
          lat: banner_street_view.data('latitude'), lng: banner_street_view.data('longitude')
        },
        pov: {
          heading: banner_street_view.data('heading'), pitch: banner_street_view.data('pitch')
        },
  ...
  if (listingGalleryPreview.length != 0) {
    listingGalleryPreview.owlCarousel({
      ...
      nav: (
      listingGalleryPreviewCount > listingGalleryPreviewItems),
      navText: [
        '<i class="fa fa-chevron-left"></i>',
        '<i class="fa fa-chevron-right"></i>'
      ]
  ...
  listingGallery.on('changed.owl.carousel', function(event) {
    ...
    if (firstActiveIndex >= currentIndex) {
      for (var i = 0; i <= (
      firstActiveIndex - currentIndex); i++) {
        listingGalleryPreview.trigger('prev.owl.carousel');
      }
    } else if (lastActiveIndex <= currentIndex) {
      for (var i = 0; i <= (
      currentIndex - lastActiveIndex); i++) {
  ...
  $('.listing-detail-gallery-preview-inner .owl-item').click(function() {
    ...
    listingGallery.trigger('to.owl.carousel', [
      itemIndex,
      300
    ]);
  });
  ...
  $('.listing-detail-gallery a').colorbox({
    ref: 'listing-gallery', maxHeight: '90%', maxWidth: '85%'
  });
  ...
  function responsiveMenu() {
    var parent_menu = $('#menu-main-menu').find('.menu-item-has-children');
