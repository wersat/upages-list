...
class parsedown
{
    ...
    private static $strongRegex
                          = [
        '*' => '/^[*]{2}((?:[^*]|[*][^*]*[*])+?)[*]{2}(?![*])/s',
        '_' => '/^__((?:[^_]|_[^_]*_)+?)__(?!_)/us',
                          ];
    private static $emRegex
                          = [
        '*' => '/^[*]((?:[^*]|[*][*][^*]+?[*][*])+?)[*](?![*])/s',
    ...
    // 
    // Private
    private static $textLevelElements
                              = [
        'a',
        'br',
        'bdo',
    ...
    private function findBlocks(array $lines, $blockContext = null)
    {
        ...
        foreach ($lines as $line) {
            ...
            case 'li':
                ...
                if ($contextData['indentation'] === $indentation and preg_match(
                    '/^' . $contextData['marker'] . '[ ]+(.*)/',
                    $line, $matches
                )
                ) {
            ...
            case 'paragraph':
                ...
                if (strpos($line, '|') !== false and strpos($block['content'], '|') !== false and chop(
                    $line,
                    ' -:|'
                ) === ''
                ) {
    ...
    private function parseLine($text, $markers = ["  \n", '![', '&', '*', '<', '[', '\\', '_', '`', 'http', '~~'])
    {
        if (isset($text[1]) === false or $markers === []) {
            return $text;
        }
        // ~
        $markup = '';
        while ($markers) {
            ...
            case '[':
                if (strpos($text, ']') and preg_match('/\[((?:[^][]|(?R))*)\]/', $text, $matches)) {
                    ...
                    if ($remainingText[0] === '(' and preg_match(
                        '/\([ ]*(.*?)(?:[ ]+[\'"](.+?)[\'"])?[ ]*\)/',
                        $remainingText, $matches
                    )
                    ) {
            ...
            case '*':
            case '_':
                if ($text[1] === $closestMarker and preg_match(
                    self::$strongRegex[$closestMarker], $text,
                    $matches
                )
                ) {
