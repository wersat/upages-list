...
class VP_Option
{
    ...
    public function __construct(array $configs)
    {
        ...
        if (isset($option_key)) {
            ...
            throw new Exception(__('Option Key is required', 'upages'), 1);
        }
        if (isset($template)) {
            $this->set_template($template);
        } else {
            throw new Exception(__('Template Array/File is required', 'upages'), 1);
        }
        if (isset($page_slug)) {
            $this->set_page_slug($page_slug);
        } else {
            throw new Exception(__('Page Slug is required', 'upages'), 1);
        }
        // swim in the pool
        self::$pool[$this->get_option_key()] = &$this;
        // check and set the remaining configs
        if (isset($menu_page)) {
            $this->set_menu_page($menu_page);
        }
        if (isset($is_dev_mode)) {
            $this->is_dev_mode($is_dev_mode);
        }
        if (isset($use_util_menu)) {
            $this->use_util_menu($use_util_menu);
        }
        if (isset($use_auto_group_naming)) {
            $this->use_auto_group_naming($use_auto_group_naming);
        }
        if (isset($minimum_role)) {
            $this->set_minimum_role($minimum_role);
        }
        if (isset($page_title)) {
        ...
        if (isset($menu_label)) {
            $this->set_menu_label($menu_label);
        }
        // add first_activation hook to save initial values to db
        add_action('vp_option_first_activation', [$this, 'initial_db_setup']);
    ...
    public function setup()
    {
        ...
        if ($this->is_dev_mode()) {
            add_action('admin_notices', [$this, 'dev_mode_notice']);
        }
    }
    ...
    public function init_options_set()
    {
        ...
        if ($this->use_util_menu()) {
            ...
            $util_menu->set_title(__('Utility', 'upages'));
            ...
            $restore_section->set_title(__('Restore Default', 'upages'));
            ...
            $exim_section->set_title(__('Export/Import', 'upages'));
    ...
    public function ajax_save()
    {
        ...
        if ($result['status']) {
            ...
            $this->get_options_set()
                ->populate_values($option, true);
            // get back options from set
            $opt = $this->get_options_set()
                ->get_values();
    ...
    {
        // do saving
        $result = $this->get_options_set()
            ->save($this->get_option_key());
        // re-init $opt
        $this->init_options_from_db();
        // get the new $opt
        $opt = $this->get_options_set()
    ...
    public function ajax_restore()
    {
        ...
        if ($result['status']) {
            ...
            do_action(
                'vp_option_after_ajax_restore', $options, $old_opt, $result['status'],
                $this->get_option_key()
            );
            // after ajax restore action hook
            do_action(
                'vp_option_after_ajax_restore-' . $this->get_option_key(), $options, $old_opt,
                $result['status']
            );
        }
        if (ob_get_length()) {
    ...
    public function ajax_export_option()
    {
        global $wpdb;
        $sr_options = null;
        $db_options = null;
        ...
        do_action(
            'vp_option_after_ajax_export', $db_options, $sr_options, $result['status'],
            $this->get_option_key()
        );
        // after ajax export action hook
        do_action(
            'vp_option_after_ajax_export-' . $this->get_option_key(), $db_options, $sr_options,
            $result['status']
        );
        if (ob_get_length()) {
            ob_clean();
