<?php
    ...
    class VP_Metabox_Depsloader
    {
        ...
        public function build()
        {
            ...
            $script_always = VP_Util_Config::instance()
                                                           ->load('dependencies', 'scripts.always');
            $style_always = VP_Util_Config::instance()
                                ...
                                foreach ($fields as $field) {
                                    if ($field['type'] == 'group') {
                                        ...
                                        if (array_key_exists($field['type'], $rules)) {
                                            $result['scripts'] = array_merge($result['scripts'],
                                                $rules[$field['type']]['js']);
                                            $result['styles'] = array_merge($result['styles'],
                                                $rules[$field['type']]['css']);
                                        }
