    ...
    class VP_Option_Control_Set
    {
        ...
        public function save($option_key)
        {
            ...
            if (update_option($option_key, $opt)) {
                ...
                $result['message'] = __('Saving successful', 'upages');
                $curr_opt = get_option($option_key, []);
            } else {
                $curr_opt = get_option($option_key, []);
                ...
                if ($changed) {
                    ...
                    $result['message'] = __('Saving failed', 'upages');
                } else {
                    $result['status'] = true;
                    $result['code'] = self::SAVE_NOCHANGES;
                    $result['message'] = __('No changes made', 'upages');
                }
            }
