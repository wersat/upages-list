...
jQuery.fn.getAttributes = function() {
    ...
    if (!this.length) {
        return this; }
    jQuery.each(
        this[0].attributes, function(index, attr) {
...
vp.custom_check_radio_event = function(parent, selector){
    jQuery(parent).delegate(
        selector, "click", function(e){
            e.preventDefault();
            var $control = jQuery(this).find('input'),
            type = $control.attr('type');
            if(type == 'radio') {
                jQuery(this).parent().parent().find('input').each(
                    function(i){
        ...
        ajaxurl, data, function(response) {
            ...
            if (response.status) {
                ...
                case 'vp-multiselect':
                    ...
                    response.data !== null && jQuery.each(
                        response.data, function(key, value) {
                            $source
                            .append(
    ...
    if (jQuery.fn.slider) {
        $elements.each(
            function(i, el) {
                ...
                $slider.prev('.slideinput').keypress(
                    function(e) {
