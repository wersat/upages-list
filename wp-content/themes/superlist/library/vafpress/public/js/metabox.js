;(function($) {
    ...
    $(document).on(
        'click', '.vp-wpa-group-title', function(e){
            e.preventDefault();
            ...
            if(control.hasClass('vp-hide')) {
                if(siblings.exists()) {
                    siblings.each(
                        function(i, el){
                            $(this).find('.vp-controls').first().slideUp(
                                'fast', function() {
        ...
        $('.wpa_loop.vp-sortable').sortable(
            {
                ...
                start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                    if(typeof window.KIA_metabox !== 'undefined') {
                        textareaIDs = [];
                        vp.tinyMCE_save();
                        $(ui.item).find('.customEditor textarea').each(
                            function(){
                                if($(this).parents('.tocopy').length <= 0) {
    ...
    $('#post').on(
        'submit', function(e){
            ...
            $(this).find('.vp-toggle .vp-input').each(
                function(){
