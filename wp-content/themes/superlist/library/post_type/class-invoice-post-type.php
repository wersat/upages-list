...
class Invoice_Post_Type
{
    ...
    public function setPostType()
    {
        ...
        $this->post_type->populate_column(
            'type', function ($column, $post) {
                $type  = get_post_meta(get_the_ID(), $this->post_type_prefix . 'type', true);
                $types = apply_filters('inventor_invoices_types', []);
                echo empty($types[$type]) ? $type : $types[$type];
            }
        );
        $this->post_type->populate_column(
            'customer', function ($column, $post) {
                echo get_post_meta(get_the_ID(), $this->post_type_prefix . 'customer_name', true);
            }
        );
        $this->post_type->populate_column(
            'total', function ($column, $post) {
                echo $this->get_invoice_total(get_the_ID());
            }
        );
        $this->post_type->populate_column(
            'currency', function ($column, $post) {
                echo get_post_meta(get_the_ID(), $this->post_type_prefix . 'currency_code', true);
            }
        );
        $this->post_type->populate_column(
            'issue_date', function ($column, $post) {
    ...
    public function fields()
    {
        ...
        $payment = new_cmb2_box(
            [
            'id'           => $this->post_type_prefix . 'payment',
        ...
        $payment->add_field(
            ...
            'id'   => $this->post_type_prefix . 'currency_code',
            'type' => 'text_small'
            ]
        );
        $payment->add_field(
            ...
            'id'   => $this->post_type_prefix . 'reference',
            'type' => 'text'
            ]
        );
        $payment->add_field(
            ...
            'id'          => $this->post_type_prefix . 'payment_term',
            'type'        => 'text_small',
        ...
        $payment->add_field(
            ...
            'id'   => $this->post_type_prefix . 'details',
            'type' => 'textarea'
            ]
        );
        $supplier = new_cmb2_box(
            [
            'id'           => $this->post_type_prefix . 'supplier',
        ...
        $supplier->add_field(
            ...
            'id'   => $this->post_type_prefix . 'supplier_name',
            'type' => 'text'
            ]
        );
        $supplier->add_field(
            ...
            'id'   => $this->post_type_prefix . 'supplier_registration_number',
            'type' => 'text_medium'
            ]
        );
        $supplier->add_field(
            ...
            'id'   => $this->post_type_prefix . 'supplier_vat_number',
            'type' => 'text_medium'
            ]
        );
        $supplier->add_field(
            ...
            'id'          => $this->post_type_prefix . 'supplier_details',
            'type'        => 'textarea'
            ]
        );
        $supplier = new_cmb2_box(
            [
            'id'           => $this->post_type_prefix . 'customer',
        ...
        $supplier->add_field(
            [
            'name' => __('Name', 'inventor-invoices'),
            'id'   => $this->post_type_prefix . 'customer_name',
            'type' => 'text'
            ]
        );
        $supplier->add_field(
            [
            'name' => __('Registration No.', 'inventor-invoices'),
            'id'   => $this->post_type_prefix . 'customer_registration_number',
            'type' => 'text_medium'
            ]
        );
        $supplier->add_field(
            [
            'name' => __('VAT No.', 'inventor-invoices'),
            'id'   => $this->post_type_prefix . 'customer_vat_number',
            'type' => 'text_medium'
            ]
        );
        $supplier->add_field(
            ...
            'name'        => __('Details', 'inventor-invoices'),
            'description' => __('for example address or any additional information', 'inventor-invoices'),
            'id'          => $this->post_type_prefix . 'customer_details',
            'type'        => 'textarea'
            ]
        );
        $cmb   = new_cmb2_box(
            [
            'id'           => $this->post_type_prefix . 'items',
        ...
        $cmb->add_group_field(
            $group, [
            'id'   => $this->post_type_prefix . 'item_title',
        ...
        $cmb->add_group_field(
            $group, [
            'id'         => $this->post_type_prefix . 'item_quantity',
        ...
        $cmb->add_group_field(
            $group, [
            'id'         => $this->post_type_prefix . 'item_unit_price',
        ...
        $cmb->add_group_field(
            $group, [
            'id'          => $this->post_type_prefix . 'item_tax_rate',
    ...
    public function customizations($wp_customize)
    {
        ...
        $wp_customize->add_setting(
            'inventor_invoices_payment_term', [
            'default'           => 15,
            'capability'        => 'edit_theme_options',
            'sanitize_callback' => 'sanitize_text_field'
            ]
        ...
        $wp_customize->add_setting(
            'inventor_invoices_billing_name', [
            'default'           => null,
            'capability'        => 'edit_theme_options',
            'sanitize_callback' => 'sanitize_text_field'
            ]
        ...
        $wp_customize->add_setting(
            'inventor_invoices_billing_registration_number', [
            'default'           => null,
            'capability'        => 'edit_theme_options',
            'sanitize_callback' => 'sanitize_text_field'
            ]
        ...
        $wp_customize->add_setting(
            'inventor_invoices_billing_vat_number', [
            'default'           => null,
            'capability'        => 'edit_theme_options',
            'sanitize_callback' => 'sanitize_text_field'
            ]
        ...
        $wp_customize->add_setting(
            'inventor_invoices_billing_details', [
            'default'           => null,
            'capability'        => 'edit_theme_options',
    ...
    {
        $items = get_post_meta($invoice_id, $this->post_type_prefix . 'item', true);
        $total = 0;
        if (! is_array($items)) {
            return $total;
        }
    ...
    {
        $unit_price_key = $this->post_type_prefix . 'item_unit_price';
        $unit_price     = ( ! empty($item[$unit_price_key])) ? (float)$item[$unit_price_key] : 0;
        $quantity_key   = $this->post_type_prefix . 'item_quantity';
        $quantity       = ( ! empty($item[$quantity_key])) ? (float)$item[$quantity_key] : 0;
        $tax_rate_key   = $this->post_type_prefix . 'item_tax_rate';
    ...
    public function catch_payment(
        ...
        $tax_rate        = apply_filters(
            'inventor_invoices_tax_rate', $tax_rate, $supplier_vat_number,
            $customer_vat_number
        );
        ...
        $item            = [
        $this->post_type_prefix . 'item_title'      => get_the_title($object_id),
        $this->post_type_prefix . 'item_quantity'   => 1,
        $this->post_type_prefix . 'item_unit_price' => (string)$item_unit_price,
        $this->post_type_prefix . 'item_tax_rate'   => (string)$item_tax_rate
        ];
        $items           = [$item];
        // details
        $details = '';
        ...
        // update invoice
        update_post_meta($invoice_id, $this->post_type_prefix . 'type', $type);
        update_post_meta($invoice_id, $this->post_type_prefix . 'currency_code', $currency_code);
        update_post_meta($invoice_id, $this->post_type_prefix . 'reference', $reference);
        update_post_meta($invoice_id, $this->post_type_prefix . 'payment_term', $payment_term);
        update_post_meta($invoice_id, $this->post_type_prefix . 'details', $details);
        update_post_meta($invoice_id, $this->post_type_prefix . 'supplier_name', $supplier_name);
        update_post_meta(
            $invoice_id, $this->post_type_prefix . 'supplier_registration_number',
            $supplier_registration_number
        );
        update_post_meta($invoice_id, $this->post_type_prefix . 'supplier_vat_number', $supplier_vat_number);
        update_post_meta($invoice_id, $this->post_type_prefix . 'supplier_details', $supplier_details);
        update_post_meta($invoice_id, $this->post_type_prefix . 'customer_name', $customer_name);
        update_post_meta(
            $invoice_id, $this->post_type_prefix . 'customer_registration_number',
            $customer_registration_number
        );
        update_post_meta($invoice_id, $this->post_type_prefix . 'customer_vat_number', $customer_vat_number);
        update_post_meta($invoice_id, $this->post_type_prefix . 'customer_details', $customer_details);
        update_post_meta($invoice_id, $this->post_type_prefix . 'item', $items);
        if ($gateway == 'wire-transfer') {
    ...
    public function invoice_number_exists($invoice_number)
    {
        ...
        = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT DISTINCT ID FROM {$wpdb->posts} WHERE post_status = \"publish\" AND post_title = '%s'",
                $invoice_number
            )
        );
            ...
            <section id="primary" class="content-area">
              <main id="main" class="site-main content" role="main">
                ...
                <div class="invoice">
                    ...
                    $type                         = get_post_meta($invoice_id, $this->post_type_prefix . 'type', true);
                    $currency_code                = get_post_meta(
                        $invoice_id, $this->post_type_prefix . 'currency_code',
                        true
                    );
                    $reference                    = get_post_meta(
                        $invoice_id, $this->post_type_prefix . 'reference',
                        true
                    );
                    $payment_term                 = get_post_meta(
                        $invoice_id, $this->post_type_prefix . 'payment_term',
                        true
                    );
                    $details                      = get_post_meta($invoice_id, $this->post_type_prefix . 'details', true);
                    $supplier_name                = get_post_meta(
                        $invoice_id, $this->post_type_prefix . 'supplier_name',
                        true
                    );
                    $supplier_registration_number = get_post_meta(
                        $invoice_id,
                        $this->post_type_prefix . 'supplier_registration_number', true
                    );
                    $supplier_vat_number          = get_post_meta(
                        $invoice_id,
                        $this->post_type_prefix . 'supplier_vat_number', true
                    );
                    $supplier_details             = get_post_meta(
                        $invoice_id,
                        $this->post_type_prefix . 'supplier_details', true
                    );
                    $customer_name                = get_post_meta(
                        $invoice_id, $this->post_type_prefix . 'customer_name',
                        true
                    );
                    $customer_registration_number = get_post_meta(
                        $invoice_id,
                        $this->post_type_prefix . 'customer_registration_number', true
                    );
                    $customer_vat_number          = get_post_meta(
                        $invoice_id,
                        $this->post_type_prefix . 'customer_vat_number', true
                    );
                    $customer_details             = get_post_meta(
                        $invoice_id,
                        $this->post_type_prefix . 'customer_details', true
                    );
                    $items                        = get_post_meta($invoice_id, $this->post_type_prefix . 'item', true);
                  ...
                  <section id="general">
                    <dl>
                      ...
                      <dd><?= _n(
                          sprintf('%d day', $payment_term), sprintf('%d days', $payment_term), $payment_term,
                          'inventor-invoices'
                      ); ?></dd>
                    ...
                    <?php if ($items_count > 0) : ?>
                      <table class="invoice-items-table">
                        ...
                        <tbody>
                            ...
                            <?php
                            $title      = $item[$this->post_type_prefix . 'item_title'];
                            $quantity   = $item[$this->post_type_prefix . 'item_quantity'];
                            $unit_price = $item[$this->post_type_prefix . 'item_unit_price'];
                            $tax_rate   = $item[$this->post_type_prefix . 'item_tax_rate'];
                  ...
                  <section id="total">
                    <label><?= __('Total', 'inventor-invoices') . ':' ?></label> <span><?php printf(
                        '%s %s', $total,
                        $currency_code
                    ); ?>
                      <span>
      ...
      <table class="invoices-table">
        ...
        <tbody>
            ...
            <tr>
              ...
              <td>
                <?= get_post_meta(get_the_ID(), $this->post_type_prefix . 'currency_code', true); ?>
              </td>
              <td>
                <?= get_the_date('Y-m-d'); ?>
              </td>
    ...
    public function get_invoice_item_list($invoice_id)
    {
        ...
        $result = [];
        foreach ((array)$items as $item) {
            $item_title_key = $this->post_type_prefix . 'item_title';
