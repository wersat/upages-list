...
class Custom_Post_Type
{
                    ...
                    if (is_array($this->taxonomies)) {
                        foreach ((array)$this->taxonomies as $tax) {
                            if ($tax !== 'category' && $tax !== 'post_tag') {
                                ...
                                $new_columns[$tax] = sprintf(
                                    __('%s', $this->textdomain),
            ...
            if (! empty($terms)) {
                ...
                foreach ((array)$terms as $term) {
                    $output[] = sprintf(
                        ...
                        esc_url(
                            add_query_arg(
                                ['post_type' => $post->post_type, $column => $term->slug],
                                'edit.php'
                            )
                        ),
                    ...
                    if ($terms) {
                        ...
                        foreach ((array)$terms as $term) {
                            if (isset($_GET[$tax_slug]) && $_GET[$tax_slug] === $term->slug) {
                                printf(
                                    '<option value="%s" selected="selected">%s (%s)</option>', $term->slug,
                                    $term->name, $term->count
                                );
                            } else {
                                printf(
                                    '<option value="%s">%s (%s)</option>', $term->slug, $term->name,
                                    $term->count
                                );
                            }
                        }
    ...
    public function sortable(array $columns = [])
    {
        ...
        $this->add_filter(
            'manage_edit-' . $this->post_type_name . '_sortable_columns',
    ...
    public function updated_messages($messages)
    {
        ...
        $messages[$this->post_type_name] = [
            ...
            5  => isset($_GET['revision']) ? sprintf(
                __('%2$s restored to revision from %1$s', $this->textdomain),
                wp_post_revision_title((int)$_GET['revision'], false), $singular
            ) : false,
            ...
            9  => sprintf(
                ...
                date_i18n(__('M j, Y @ G:i', $this->textdomain), strtotime($post->post_date)), $singular
            ),
    ...
    public function bulk_updated_messages($bulk_messages, $bulk_counts)
    {
        ...
        $bulk_messages[$this->post_type_name] = [
            'updated'   => _n(
                '%s ' . $singular . ' updated.', '%s ' . $plural . ' updated.',
                $bulk_counts['updated']
            ),
            'locked'    => _n(
            ...
            'trashed'   => _n(
                '%s ' . $singular . ' moved to the Trash.', '%s ' . $plural . ' moved to the Trash.',
                $bulk_counts['trashed']
            ),
            'untrashed' => _n(
